...
- Пошук закладів (`search_places`) поруч із пошуком подій на мапі й головній: секція «Місця», тап веде мапу до закладу й відкриває його стос. «Ще в цьому місці» для афіші береться з `place_events`, а не з завантаженого індексу; індекс лишається запасним для спільнотних подій і офлайну.
