...
import app.poruch.data.secureRandomBytes
import kotlin.io.encoding.Base64
import kotlin.io.encoding.ExperimentalEncodingApi

/**
 * PKCE для листів підтвердження й відновлення (RFC 7636). Посилання з листа несе одноразовий
 * `code`, а не живі токени: перехопити custom scheme `poruch://` може будь-який застосунок на
 * пристрої, і без `code_verifier`, який лишається в захищеному сховищі, код нічого не вартий.
 * SHA-256 написано тут, бо Ktor не дає спільного дайджесту для всіх таргетів, а тягнути
 * криптобібліотеку заради 64 рядків не варто.
 */
internal object Pkce {
    /** Випадкові байти криптографічної якості: SecureRandom на JVM/Android, SecRandomCopyBytes на iOS. */
    fun newVerifier(): String = base64Url(secureRandomBytes(32))
