...
class LocalReminderPreference(private val database: PoruchDatabase) : ReminderPreferenceStore {
    override fun enabled(): Boolean = database.cacheQueries.readDevice(KEY).executeAsOneOrNull() == "true"
    override fun setEnabled(enabled: Boolean) { database.cacheQueries.writeDevice(KEY, enabled.toString()) }
