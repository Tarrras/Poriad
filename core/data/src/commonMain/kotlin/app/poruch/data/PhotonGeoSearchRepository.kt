...
class PhotonGeoSearchRepository(private val client: HttpClient, private val endpoint: String = "https://photon.komoot.io/api/"): GeoSearchRepository {
    override suspend fun search(query: String): List<CityResult> {
        ...
        try {
            ...
            if (response.status.value !in 200..299) throw AppException(Failure.NETWORK,"Пошук міста тимчасово недоступний")
        ...
        catch (e: Exception) { throw AppException(Failure.NETWORK,"Не вдалося знайти місто. Спробуйте ще раз") }
    }
}
