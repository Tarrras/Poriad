...
class PersistentCreationIdentity(private val database: PoruchDatabase, private val auth: AuthRepository): CreationIdentityStore {
    ...
    override fun idFor(draft: EventDraft): String {
        ...
        val previous=database.cacheQueries.readDevice(key()).executeAsOneOrNull()?.let { runCatching { Json.parseToJsonElement(it).jsonObject }.getOrNull() }
        if(previous?.string("fingerprint")==fingerprint) return previous.string("id")
        val id=Uuid.random().toString()
        database.cacheQueries.writeDevice(key(),buildJsonObject { put("fingerprint",fingerprint); put("id",id) }.toString())
        return id
    }
    override fun clear() { database.cacheQueries.writeDevice(key(),"{}") }
}
