...
from . import emit, normalize, report
...
def run_city(city: str, sources: list, run_id: str, *,
             refresh_osm: bool = False, use_photon: bool = True) -> tuple[list, list[str]]:
    ...
    for source in usable:
        items, counters = harvest(source, city, index, geocoder=geocoder)
        all_items += items
        harvested.append((source, items))
    ...
    sql_parts: list[str] = []
    for source, items in harvested:
        if items:
            sql_parts += ([emit.venues_sql(items, city)]
                          + emit.events_sql(items, run_id)
                          + [emit.retire_sql(source.slug, run_id, city)])
    return all_items, sql_parts


def _write_sql(path: pathlib.Path, run_id: str, parts: list[str], count: int,
               max_bytes: int = 0) -> None:
    """Записує SQL одним файлом або кількома, якщо заданий розмір.

    Нарізка потрібна не базі, а SQL Editor у Supabase: на файлі в мегабайт він відповідає «Query
    is too large to be run via the SQL Editor». Межа проходить ТІЛЬКИ між цілими командами —
    `parts` для того й лишається списком. Різати готовий текст по «;» не можна: крапка з комою
    трапляється всередині описів подій.

    Кожна частина — самодостатня транзакція. Застосовувати треба по порядку номерів: команди
    зняття з публікації йдуть після вставок свого джерела, і зміна порядку зняла б щойно
    вставлене.
    """
    used = [p for p in parts if p]
    head = (f"-- згенеровано tools.ingest, run_id={run_id}\n"
            "-- Застосовувати свідомо: перевірте docs/event-discovery.md перед першим запуском.\n")

    if max_bytes <= 0:
        path.write_text(head + "begin;\n" + "".join(used) + "commit;\n", "utf-8")
        print(f"  SQL: {path}  ({count} подій)")
        return
    ...
    for number, chunk in enumerate(chunks, 1):
        out = path.with_name(f"{path.stem}.{number:0{width}d}{path.suffix}")
        out.write_text(
            head + f"-- частина {number} з {len(chunks)}. Застосовувати по порядку номерів.\n"
            "begin;\n" + "".join(chunk) + "commit;\n", "utf-8")
        print(f"  SQL {number}/{len(chunks)}: {out}  ({len(out.read_bytes()) // 1024} КБ)")
    print(f"  разом {count} подій у {len(chunks)} частинах")
...
def main(argv: list[str] | None = None) -> int:
    ...
    sources = [by_slug(s) for s in args.source] if args.source else enabled_sources()
    if not sources:
